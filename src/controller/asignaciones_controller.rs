...
use sqlx::PgPool;
use crate::models::asignaciones::CrearAsignacion;
use crate::service::asignaciones_service as service;

pub fn asignaciones_router(pool: PgPool) -> Router {
    Router::new()
        .route("/asignaciones", get(listar).post(crear))
        .route("/asignaciones/{id}", get(buscar).put(actualizar).delete(eliminar))
        .with_state(pool)
}
...
async fn crear(State(pool): State<PgPool>, Json(datos): Json<CrearAsignacion>) -> Json<serde_json::Value> {
    Json(serde_json::json!(service::crear(&pool, datos).await))
}

async fn actualizar(State(pool): State<PgPool>, Path(id): Path<i32>, Json(datos): Json<CrearAsignacion>) -> Json<serde_json::Value> {
...
async fn eliminar(State(pool): State<PgPool>, Path(id): Path<i32>) -> Json<serde_json::Value> {
    ...
    if eliminado {
        Json(serde_json::json!({"mensaje": "Asignacion eliminada correctamente"}))
    } else {
